function multiUploader(config){
        ...
        if(this.items.length > 0){
            ...
            for(var i = 0; i<this.items.length; i++){
                ...
                if(typeof this.items[i] != undefined){
                    ...
                    html += '<div class="dfiles'+errorClass+'" rel="'+uId+'"><h5>'+sampleIcon+this.items[i].name+'</h5><div id="'+uId+'" class="progress"></div>';
                }
            }
    ...
    multiUploader.prototype._uploader = function(file,f){
        ...
        if(typeof file[f] != undefined && self._validate(file[f].type) > 0 && !file[f].finished){
            console.log(file[f]);
            var data = new FormData();
            var ids = file[f].name._unique();
            data.append('file',file[f]);
            data.append('index',ids);
            data.append('email', this.config.email);
                ...
                xhr: function() { 
                    ...
                    xhr.upload.addEventListener("progress", function(evt){
                        ...
                        if (evt.lengthComputable) {  
                            ...
                            console.log(percentComplete + ids);
                            $('#' + ids).css("width", percentComplete + "%");
                        }
                    }, false);
                    return xhr;
                },
